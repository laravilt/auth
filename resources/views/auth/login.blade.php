    ...
    <div class="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8">
            ...
            <div class="bg-white dark:bg-slate-800 shadow-xl rounded-2xl overflow-hidden" x-data="{
                ...
                showPassword: false,
                loading: false,
                enabledMethods: {{ Js::from(config('laravilt.auth.login_methods', ['email'])) }}
            }">
                <!-- Method Tabs (if multiple methods enabled) -->
                <template x-if="enabledMethods.length > 1">
                    <div class="border-b border-slate-200 dark:border-slate-700">
                        <nav class="flex -mb-px" role="tablist">
                            <template x-if="enabledMethods.includes('email')">
                                <button
                                    @click="loginMethod = 'email'"
                                    :class="loginMethod === 'email' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400'"
                            ...
                            <template x-if="enabledMethods.includes('phone')">
                                <button
                                    @click="loginMethod = 'phone'"
                                    :class="loginMethod === 'phone' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400'"
                                    class="flex-1 py-4 px-1 text-center border-b-2 font-medium text-sm transition-colors"
                                    role="tab"
                                >
                                    <svg class="w-5 h-5 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <template x-if="enabledMethods.includes('username')">
                                <button
                                    @click="loginMethod = 'username'"
                                    :class="loginMethod === 'username' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400'"
                                    class="flex-1 py-4 px-1 text-center border-b-2 font-medium text-sm transition-colors"
                                    role="tab"
                                >
                                    <svg class="w-5 h-5 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-8">
                    <form method="POST" action="{{ route('laravilt.auth.login') }}" @submit="loading = true">
                        @csrf
                        ...
                        <!-- Phone Login -->
                        <div x-show="loginMethod === 'phone'" x-cloak>
                            <div class="space-y-6">
                                <div>
                        ...
                        <!-- Username Login -->
                        <div x-show="loginMethod === 'username'" x-cloak>
                            <div class="space-y-6">
                                <div>
                        ...
                        <!-- Password Field (Common for all methods) -->
                        <div class="mt-6">
                        ...
                        <div class="flex items-center justify-between mt-6">
                            ...
                            <div class="text-sm">
                                <a href="{{ route('laravilt.auth.password.request') }}" class="font-medium text-blue-600 hover:text-blue-500 dark:text-blue-400 dark:hover:text-blue-300 transition-colors">
                                    {{ __('Forgot password?') }}
                                </a>
                            </div>
                        </div>
